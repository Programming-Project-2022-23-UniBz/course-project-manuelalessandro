...
public class RoomControlTest {
    ...
    public void testPullData() {
        ...
        assertTrue(RoomControl.getRoomsArray().length > 0, "Rooms array should not be empty");
    }
    ...
    public void testPushData() {
        ...
        assertTrue(Files.exists(Paths.get(filePath)), "rooms.json file should exist");
        ...
        try {
            ...
            assertEquals(expectedJson, lastLine, "JSON data in file should contain the newly added rooms");
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void testGetRoom() {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("An exception occurred while retrieving a valid room: " + e.getMessage());
        }
        assertEquals(expectedRoom, retrievedRoom, "Retrieved room should match the expected room");

        // Test with an invalid room ID
        int invalidId = -784;
        try {
            RoomControl.getRoom(invalidId);
            System.out.println("Expected IllegalArgumentException to be thrown for an invalid room ID");
        } catch (IllegalArgumentException e) {
