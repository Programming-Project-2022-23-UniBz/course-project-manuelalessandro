 ...
 * It could be used in the same way as LinearGradientPaint and
 * RadialGradientPaint
 * and follows the same syntax.
 * You could use floats from 0.0 to 1.0 for the fractions which is standard but
 * it's
 * also possible to use angles from 0.0 to 360 degrees which is most of the
 * times
 ...
 * If you would like to use degrees instead of values from 0 to 1 you have to
 * use
...
public final class ConicalGradientPaint implements java.awt.Paint {
    ...
    /**
     * Enhanced constructor which takes the FRACTIONS in degress from 0.0f to 360.0f
     * and
    ...
    public ConicalGradientPaint(final boolean USE_DEGREES, final java.awt.geom.Point2D CENTER, final float GIVEN_OFFSET,
        ...
        if (USE_DEGREES) {
            ...
            // Now it seems to work with rotation of 0.5f, below is the old code to correct
            // the problem
            // if (GIVEN_OFFSET == -0.5)
            // {
        ...
        // Adjust fractions and colors array in the case where startvalue != 0.0f and/or
        // endvalue != 1.0f
        ...
        // Sort the hashmap by fraction and add the values to the FRACION_LIST and
        // COLOR_LIST
        ...
        for (int i = 0; i < (COLORS.length - 1); i++) {
            ...
            GREEN_STEP_LOOKUP[i] = ((COLORS[i + 1].getGreen() - COLORS[i].getGreen()) * INT_TO_FLOAT_CONST)
                    / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
            BLUE_STEP_LOOKUP[i] = ((COLORS[i + 1].getBlue() - COLORS[i].getBlue()) * INT_TO_FLOAT_CONST)
                    / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
            ALPHA_STEP_LOOKUP[i] = ((COLORS[i + 1].getAlpha() - COLORS[i].getAlpha()) * INT_TO_FLOAT_CONST)
                    / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
        }
    }

    /**
     * Recalculates the fractions in the FRACTION_LIST and their associated colors
     * in the COLOR_LIST with a given OFFSET.
     * Because the conical gradients always starts with 0 at the top and clockwise
     * direction
     * you could rotate the defined conical gradient from -180 to 180 degrees which
     * equals values from -0.5 to +0.5
     * 
     * @param FRACTION_LIST
     * @param COLOR_LIST
     * @param OFFSET
     * @return Hashmap that contains the recalculated fractions and colors after a
     *         given rotation
     */
    ...
    /**
     * With the START_COLOR at the beginning and the DESTINATION_COLOR at the end of
     * the given RANGE the method will calculate
     * and return the color that equals the given VALUE.
     * e.g. a START_COLOR of BLACK (R:0, G:0, B:0, A:255) and a DESTINATION_COLOR of
     * WHITE(R:255, G:255, B:255, A:255)
     * with a given RANGE of 100 and a given VALUE of 50 will return the color that
     * is exactly in the middle of the
     * gradient between black and white which is gray(R:128, G:128, B:128, A:255)
     * So this method is really useful to calculate colors in gradients between two
     * given colors.
     * 
     * @param START_COLOR
    ...
    public java.awt.PaintContext createContext(final java.awt.image.ColorModel COLOR_MODEL,
            final java.awt.Rectangle DEVICE_BOUNDS, final java.awt.geom.Rectangle2D USER_BOUNDS,
    ...
    private final class ConicalGradientPaintContext implements java.awt.PaintContext {
        ...
        public java.awt.image.Raster getRaster(final int X, final int Y, final int TILE_WIDTH, final int TILE_HEIGHT) {
            ...
            final java.awt.image.WritableRaster RASTER = getColorModel().createCompatibleWritableRaster(TILE_WIDTH,
                    TILE_HEIGHT);
                ...
                for (int px = 0; px < TILE_WIDTH; px++) {
                    ...
                    for (int i = 0; i < (MAX - 1); i++) {
                        if ((angle >= FRACTION_ANGLES[i])) {
                            currentRed = COLORS[i].getRed() * INT_TO_FLOAT_CONST
                                    + (angle - FRACTION_ANGLES[i]) * RED_STEP_LOOKUP[i];
                            currentGreen = COLORS[i].getGreen() * INT_TO_FLOAT_CONST
                                    + (angle - FRACTION_ANGLES[i]) * GREEN_STEP_LOOKUP[i];
                            currentBlue = COLORS[i].getBlue() * INT_TO_FLOAT_CONST
                                    + (angle - FRACTION_ANGLES[i]) * BLUE_STEP_LOOKUP[i];
                            currentAlpha = COLORS[i].getAlpha() * INT_TO_FLOAT_CONST
                                    + (angle - FRACTION_ANGLES[i]) * ALPHA_STEP_LOOKUP[i];
                            continue;
                        }
                    }
